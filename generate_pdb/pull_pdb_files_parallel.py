...
def create_file_set(p):
    ...
    contents = pypdb.describe_pdb(p)
    f = open(target + '/' + p + '/DATA/' + p + '_description', 'w')
    f.write(contents)
    print('\tWriting ' + p + ' Description File')
    f.close()
...
if __name__ == '__main__':
    ...
    # protiens = pypdb.get_all()
    
    protiens = pypdb.Query('covid').search()
    sample = protiens
    #sample_size = int(sys.argv[2])

    #sample = random.sample(protiens, sample_size)
    str = ''
    for p in sample:
        str = str + p + '\n'
    ...
    p = Pool(processes=4)
    outputs = p.map(pull_and_generate, sample)	
